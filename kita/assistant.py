...
class Assistant:
    ...
    def are_similar(self, folder_name, course_key, course_name):
        ...
        if (folder_name.lower() == course_key or folder_name.startswith(course_name) or 
            course_name.startswith(folder_name)):
            return True
        course_suffixes = {'I': 1, 'II': 2, 'III': 3}
        for suffix in course_suffixes:
            if folder_name.endswith(suffix) or course_name.endswith(suffix):
                return (folder_name.replace(suffix, str(course_suffixes[suffix])) == course_name or
                    course_name.replace(suffix, str(course_suffixes[suffix])) == folder_name)
